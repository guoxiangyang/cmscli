...
var func_exists  = require('./tree/exists.js');
...
Tree.prototype.find = function (remote, opt, callback) {
    ...
    opt = merge(this.config.opt, opt);
    remote = this.add_root(remote);
    opt.stdout = this.config.stdout;
    opt.stderr = this.config.stderr;
...
Tree.prototype.select = function (remote, query, opt, callback) {
    // console.log(remote, query, opt, callback);
    if (typeof opt === 'function') {
        callback = opt;
        opt = {};
    }
...
Tree.prototype.exists = function (tree, opt, callback) {
    if (typeof opt === 'function') {
        callback = opt;
        opt = {};
    }
    opt = merge(this.config.opt, opt);
    func_exists(this.config.host, this.config.port, tree, opt, callback);
}
module.exports = Tree;
